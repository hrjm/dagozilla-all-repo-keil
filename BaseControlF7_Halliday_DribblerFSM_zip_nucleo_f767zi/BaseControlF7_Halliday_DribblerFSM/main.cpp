...
//        nh.spinOnce();
        ...
        cur_locomotion_R = -locomotionEncR.GetCounter(1)/4096;
        cur_locomotion_L = -locomotionEncL.GetCounter(1)/4096;
        cur_locomotion_B = -locomotionEncB.GetCounter(1)/4096;
...
void moveDribbler(){
    ...
    if(dribbler_state){
        ...
        //Constants
        float stable_pwm = 0.3;
        float first_movement_tolerance = 0.2;
        float second_movement_tolerance = 0.8;
        float second_gain = 0.1;
        float first_gain = 0.45;
        float right_dribbler_pwm, left_dribbler_pwm;
        float pwm_limit = 0.12;
        
        // conditional for vy
        if(vy > second_movement_tolerance){
            left_from_vy = second_gain;
            right_from_vy = second_gain;
            
            right_dribbler_pwm = pwm_limit - right_from_vy*(vy-second_movement_tolerance);
            left_dribbler_pwm = -pwm_limit + left_from_vy*(vy-second_movement_tolerance);
            
            //Clamping
            if(right_dribbler_pwm > pwm_limit) right_dribbler_pwm = pwm_limit;
            else if(right_dribbler_pwm < -stable_pwm) right_dribbler_pwm = -stable_pwm;
            
            if(left_dribbler_pwm > stable_pwm) left_dribbler_pwm = stable_pwm;
            else if(left_dribbler_pwm < -pwm_limit) left_dribbler_pwm = -pwm_limit;
        }
        else if(vy > first_movement_tolerance){
            left_from_vy = first_gain;
            right_from_vy = first_gain;
            
            right_dribbler_pwm = -(stable_pwm - right_from_vy*(vy-first_movement_tolerance));
            left_dribbler_pwm = stable_pwm - left_from_vy*(vy-first_movement_tolerance);
            ...
            right_dribbler_pwm = -stable_pwm;
            left_dribbler_pwm = stable_pwm;
        }
        
        //motor pwm
        dribblerMotorR.setpwm(right_dribbler_pwm);
        dribblerMotorL.setpwm(left_dribbler_pwm);
...
//        
//        //from potensio
//        float right_v1 = (right_pot_target - cur_pot_R) * right_from_potensio;
//        float left_v1 = (left_pot_target - cur_pot_L) * left_from_potensio;
//        //error tolerance
//        if(fabs(right_pot_target - cur_pot_R)<tolerance) right_v1 = -0.3;
//        if(fabs(left_pot_target - cur_pot_L)<tolerance) left_v1 = 0.3;
//        
//        //condition that no ball in dribbler
//        if(fabs(cur_pot_L)<=0.4 && fabs(cur_pot_R)<=0.4){
//            right_vtarget = right_v1;
//            left_vtarget = left_v1;
//        } else{ //ball detected
//           //from robot velocity
//            right_vtarget = right_v1 + (right_from_vx*fabs(vx) + right_from_vy*vy + right_from_rot*vw);
//            left_vtarget = left_v1 + (left_from_vx*fabs(vx) + left_from_vy*vy + left_from_rot*vw);
//        }
//    
//        //compute PID
...
////        dribblerMotorR.setpwm(-0.4);
////        dribblerMotorL.setpwm(0.4);
////        
...
//        vw = (locomotion_R_vel + locomotion_L_vel + locomotion_B_vel)/(3*0.2);
//        
//        float v_magnitude_cur = sqrt(vx*vx + vy*vy);
//        float theta_cur = atan2(vy, vx);
//        
//        if( theta_cur < 0 ) theta_cur+=2*PI;
//        
//        float series = (theta_cur/0.261799) + 1;
//        float right_vtarget, left_vtarget;
//        float ball_threshold = 0.4;
//        float gear_ratio = 3.6;
//        float open_loop_velocity = 3.6;
//        float rotation_gain = 0.5;
//        float vy_factor = 0.5;
//       
//        //Has ball
//        right_vtarget = -vy_factor*vy - vw*rotation_gain - 4*v_magnitude_cur*(-0.00265*series*series*series + 0.0977*series*series - 0.8194*series + 0.2563)/gear_ratio;
//        left_vtarget = vy_factor*vy - vw*rotation_gain + 4*v_magnitude_cur*(-0.00265*series*series*series + 0.10925*series*series - 1.11995*series + 1.6361)/gear_ratio;
//        
//        //compute PID
//        float dribbler_R_target_rate = dribblerPidR.createpwm(right_vtarget, dribbler_R_vel);
//        float dribbler_L_target_rate = dribblerPidL.createpwm(left_vtarget, dribbler_L_vel);
//        
//        if(dribbler_R_target_rate > 1) dribbler_R_target_rate = 1;
//        else if(dribbler_R_target_rate < -1) dribbler_R_target_rate = -1;
//        
//        if(dribbler_L_target_rate > 1) dribbler_L_target_rate = 1;
//        else if(dribbler_L_target_rate < -1) dribbler_L_target_rate = -1;
//        
//        dribblerMotorR.setpwm(dribbler_R_target_rate);
//        dribblerMotorL.setpwm(dribbler_L_target_rate);
//        
////        printf("L = %.3f, B = %.3f, R = %.3f, R Pot = %.3f, L Pot = %.3f\n", locomotion_L_vel, locomotion_B_vel, locomotion_R_vel, cur_pot_R, cur_pot_L);
////        printf("R = %.3f, L = %.3f, distance = %.3f\n", dribbler_R_target_rate, dribbler_L_target_rate, distance);
//    }
//    else{
//        dribblerMotorR.setpwm(0);
//        dribblerMotorL.setpwm(0);   
