...
float theta;
float theta_prev;
//additional dribbler parameters
float vl, vr; 
float vlres = 0.0f; 
float vrres = 0.0f;
float bevelRatio = 3.6;
float speedReferenceFunc = 2.0;
float distToWheel = 0.2;
int regType;
float xResSpeed, yResSpeed;    
float actualMovingAngle;
float modifiedMovingAngle;
float v_magnitude_cur;  
float vx, vy, vw; 
float max_L_threshold = 0.2f;
float max_R_threshold = 0.2f;
float cur_L_threshold;
float cur_R_threshold;
#endif